...
public class NseRestController{
	...
	public FilteredResponse getNiftyData(Model model) {
		return processor.processNiftyData(nifty.getLiveNiftyData(FeignBuilder.builder()), 300,50);
	}
	
	@GetMapping("banknifty")
	public FilteredResponse getBankNifty(Model model) {
		return processor.processBankNiftyData(nseProxy.getLiveBankNiftyData(FeignBuilder.builder()), 600, 100);
	}
